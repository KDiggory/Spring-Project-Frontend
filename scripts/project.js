...
const showForm = () => {
    //how to select where to put it?
    const formNameLabel = document.createElement("label");
    formNameLabel.setArribute("class", "label");
    formNameLabel.setArribute("for", "plantName");
    formNameLabel.innerText("Plant Name");
    const formNameInput = document.createElement("input");
    formNameInput.setArribute("class", "label");
    formNameInput.setArribute("type", "text");
    formNameInput.setArribute("id", "nameActual");
    formNameInput.setArribute("name", "plantName");
...
const createSubmit = () => {
    ...
    const newPlant = {
        ...
        foliageColour: form.foliageActual.value,
        plantingMonth: form.monthActual.value,
        plantingPosition: form.positionActual.value,
        folwerColour: form.flowerColActual.value,
        id: form.idInputAcutal
    }
    console.log("Plant to add: ", newPlant);
}

//     axios
//     .post("${baseURL}//createPlant", newPlant)  // should enter the new plant into the db
//         .then(res => { 
//          console.log(res);
//        //  read(); // should get all entries including the new one
//          form.reset(); // resets the form
//          form.name.docus // puts cursor in name field
//         })
//         .catch(err => console.log(err));

//     const output = document.createElement("h2");
//     output.classList.add("output-text");
//     output.innerText = newPlant;
